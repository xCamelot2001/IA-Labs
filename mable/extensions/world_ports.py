...
from mable.simulation_space import Port, NetworkWithPortDict, Location, OnJourney
...
class WorldVessel(SimpleVessel):
    ...
    def __init__(self, capacities_and_loading_rates, location, speed, keep_journey_log=True, name=None):
        ...
        :type name: str
        """
        super().__init__(capacities_and_loading_rates, location, speed, keep_journey_log=keep_journey_log, name=name)
...
class LatLongShippingNetwork(NetworkWithPortDict):
    ...
    def get_journey_location(self, journey, vessel, current_time):
        ...
        if time_travelled == 0:
            ...
            route_distance = [sum(segment_distances_dist[:i]) for i in range(len(segment_distances_dist))]
