...
class TimeWindowArrivalEvent(ArrivalEvent):
    ...
    def __eq__(self, other):
        return super().__eq__(other) and self._trade.time_window == other.trade.time_window
...
class DistributionShipping(Shipping):
    ...
    def initialise_trades(self, *args, **kwargs):
        ...
        logger.debug(f"Generating trades from 0 to {simulation_length} every {trade_occurrence_frequency}."
        ...
        for i in range(0, simulation_length + 1, trade_occurrence_frequency):
            ...
            cargoes_generated = self.sample_cargoes_from_port_distributions(
                ...
                a,
                time=i)
            logger.debug(f"Generated {len(cargoes_generated)} cargoes for time {i}.")
    ...
    def sample_time_windows(world, time_transition_dict, time_transit_distribution,
        ...
        time_windows_allowance :  int, optional
        ...
        time_window = world.random.normal(transition_mean, transition_std)
    ...
    def sample_cargoes_from_port_distributions(self, world, class_factory, number_of_cargoes, cargo_weight_distribution,
                                               frequency_distribution, time_transit_distribution, pickup_period,
                                               time,
