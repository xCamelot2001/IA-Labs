...
from mable.observers import (
    LogRunner, AuctionMetricsObserver, EventFuelPrintObserver, MetricsObserver, AuctionOutcomeObserver)
...
def _activate_stats_collection(simulation, show_detailed_auction_outcome=False):
    ...
    if show_detailed_auction_outcome:
        simulation.register_event_observer(AuctionOutcomeObserver(logger))
...
def _calculate_penalty(simulation, metrics_observer):
    penalties = {
        metrics_observer.metrics.get_company_id(c, create_id_if_not_exists=False): 0
        for c in simulation.shipping_companies}
    return penalties  # TODO calculate penalty
