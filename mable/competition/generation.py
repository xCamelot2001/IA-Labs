...
class AuctionCargoEvent(CargoEvent):
    ...
    def event_action(self, engine):
        ...
        # TODO ensure that trade allocations are logged
        ...
        self._allocation_result: Dict[Trade, None | Tuple[str, float]] = {one_trade: None for one_trade in all_trades}
        for one_company in distribution_ledger.keys():
            for one_contract in distribution_ledger[one_company]:
                self._allocation_result[one_contract.trade] = (one_company.name, one_contract.payment)
        num_awarded_trades = sum(len(distribution_ledger[k]) for k in distribution_ledger.keys())
